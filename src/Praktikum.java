...
public class Praktikum {
    ...
    static class PostsHandler implements HttpHandler {
        ...
        private void handlePostComments(HttpExchange exchange) throws IOException {
            ...
            Optional<Integer> postId = getPostId(exchange);
            if (postId.equals(Optional.empty())) {
            ...
            for (Post post : posts) {
                if (post.getId() == postId.get()) {
                    InputStream responseBody = exchange.getRequestBody();
                    String responseString = new String(responseBody.readAllBytes(), DEFAULT_CHARSET);
                    Comment commentFormJson = new Comment("", "");
                   try {
                       commentFormJson  = gson.fromJson(responseString,Comment.class);
                   } catch (JsonSyntaxException e) {
                       writeResponse(exchange, "Получен некорректный JSON", 400);
                       return;
                   }
                    try {
                       if (commentFormJson.getText() == null || commentFormJson.getUser() == null ) {
                    ...
                    post.addComment(commentFormJson);
                    writeResponse(exchange, "Комментарий добавлен", 201);
                isContains = true;
                }
            }
            if (!isContains) {
                writeResponse(exchange, "Пост с идентификатором " + postId.get() + " не найден",
                        404);
            }
